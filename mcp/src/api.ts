 ...
 * Mirrors cli/src/api.ts — the CLI package exposes no importable surface
 * (it is a bin that bundles to a single file), so the locked contract is
  ...
  constructor(url: string, cause: unknown) {
    super(
      `could not reach the Promit API at ${url} (${String(cause)}). ` +
        `Is the backend running? Set PROMIT_API_URL to point elsewhere.`,
    );
